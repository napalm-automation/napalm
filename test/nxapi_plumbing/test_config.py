...
def test_config_jsonrpc_raises_NXAPICommandError_on_non_200_config_error(
    ...
    with pytest.raises(NXAPICommandError, match='The command "bogus command" gave the error'):
...
def test_config_xml_raises_NXAPIPostError_on_non_200_post_error(mock_pynxos_device_xml):
    with pytest.raises(NXAPIPostError, match="Invalid status code returned on NX-API POST"):
