...
from napalm.nxos_api import nxos_api
...
def set_device_parameters(request):
    ...
    request.cls.driver = nxos_api.NXOSAPIDriver
...
class PatchedNXOSDriver(nxos_api.NXOSAPIDriver):
    """Patched NXOS Driver."""
...
class FakeNXOSDevice(BaseTestDouble):
    ...
    def show(self, command, raw_text=False):
        """Fake show."""
        filename = "{}.xml".format(command.replace(" ", "_"))
        full_path = self.find_file(filename)
        if raw_text:
            result = self.read_txt_file(full_path)
        else:
            xml_data = self.read_txt_file(full_path)
            result = etree.fromstring(xml_data.strip())
        return result
