...
class JunOSDriver(NetworkDriver):
    ...
    def get_lldp_neighbors_detail(self, interface=""):
        ...
        rpc_call_map = {
            ...
            "MX": {"default": rpc_call_without_information},
            "M": {"default": rpc_call_without_information},
            "T": {"default": rpc_call_without_information},
            "PTX": {"default": rpc_call_without_information},
            "SWITCH": {
                "default": rpc_call_with_information,
            ...
            "SRX_HIGHEND": {"default": rpc_call_without_information},
        }
        ...
        if rpc_call_map.get(personality) is not None:
            if rpc_call_map.get(personality).get(model) is not None:
                lldp_table.GET_RPC = (
                    rpc_call_map.get(personality).get(model).get("get_rpc")
                )
                interface_variable = (
                    rpc_call_map.get(personality).get(model).get("interface_rpc")
                )
            else:
                lldp_table.GET_RPC = (
                    rpc_call_map.get(personality).get("default").get("get_rpc")
                )
                interface_variable = (
                    rpc_call_map.get(personality).get("default").get("interface_rpc")
                )
        else:
